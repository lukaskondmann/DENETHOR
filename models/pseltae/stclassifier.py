...
class PseLTae(nn.Module):
    ...
    def __init__(self, input_dim=10, mlp1=[10, 32, 64], pooling='mean_std', mlp2=[132, 128], with_extra=True,
        ...
        self.modelname = "PseLTae"

    def forward(self, x):
        """
         Args:
            input: Pixel-Set : Batch_size x Sequence length x Channel x Number of pixels

            modification: for this application, all sequences have the same length. for compatibility with other
            models, we let the model input only one tensor and define the mask as ones in this forward
        """
        # modification: create Mask in this function
        B, T, C, N = x.shape
        input = (x, torch.ones(size=(B, T, N),device=x.device))
        # end modification
...
class PseTae(nn.Module):
    ...
    def forward(self, input):
        ...
        # modification: create Mask in this function
        B, T, C, N = input.shape
        input = (input, torch.ones(size=(B, T, N),device=input.device))
        # end modification
